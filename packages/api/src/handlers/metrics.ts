...
export default async function metricsHandler(c: Context): Promise<Response> {
    ...
    try {
        ...
        if (app?.models) {
            // Session counts
            const [totalSessions, activeSessions, pendingSessions] = await Promise.all([
            ...
            gauge('moenet_sessions_total', 'Total number of BGP sessions', totalSessions);
            gauge('moenet_sessions_active', 'Number of active BGP sessions', activeSessions);
            gauge('moenet_sessions_pending', 'Number of pending BGP sessions', pendingSessions);

            // Router counts
            const routers = await app.models.routers.findAll();
            gauge('moenet_routers_total', 'Total number of routers', routers.length);

            const openRouters = routers.filter((r: RouterModel) => r.openPeering).length;
            gauge('moenet_routers_open', 'Number of routers accepting peering', openRouters);
        ...
        gauge('moenet_memory_heap_used_bytes', 'Heap memory used', memUsage.heapUsed);
        gauge('moenet_memory_heap_total_bytes', 'Total heap memory', memUsage.heapTotal);
