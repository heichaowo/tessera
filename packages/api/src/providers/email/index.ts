    ...
    private async sendViaMailgun(options: EmailOptions): Promise<EmailResult> {
        try {
            ...
            if (!response.ok) {
                ...
                logger.error('Mailgun API error', undefined, { status: response.status, body: errorText });
                return { success: false, error: `Mailgun API error: ${response.status}` };
            }

            const data = await response.json() as { id: string };
            logger.info('Email sent via Mailgun', { to: options.to, messageId: data.id });

            return { success: true, messageId: data.id };
        } catch (err) {
            logger.error('Mailgun send failed', err instanceof Error ? err : undefined);
